...
public class OrderProcessor {
	...
	public static String getCustomTariff(String productName, String country) {
		...
		if (productName.toLowerCase().contains("filter")) {
			if (country.toLowerCase().equals("uk") || country.toLowerCase().equals("gb")) {

				return FILTERS_UK_CUSTOM_TARIF;
			} else
				return FILTERS_GERENAL_CUSTOM_TARIF;
		}
		...
		for (Order order : orders) {
			...
			if (order.isRedLabel()) {
				...
				if (maxQtdToSplitStr.isPresent()) {
					...
					if (qtdToSend > qtsMax) {
						if (getPriorityBlueCountries().contains(order.getShipCountry().toLowerCase())
	...
	private static HashSet<String> getPriorityBlueCountries() {
		return new HashSet<>(Arrays.asList(
				"uk", "gb", "be", "bg", "hr", "cz", "dk", "ee", "fi", "fr", "de", "ge", "gi", "gr", "gl", "hu", "is", "it", "lt", "lu", "nl", "no", "pl", "pt", "ro", "sk", "si", "sz", "se"
		));
	}
}
